...
export default class BIG_NUMBER extends HTMLElement {
  ...
  get style() { return `<style>${BIG_NUMBER_STYLES}</style>`; }
  get template() { return `${BIG_NUMBER_TEMPLATE}`; }
...
_applyStateToDOM() { 
  ...
  this.sdChangeValue.textContent = this._formatSigned(this.state.changeValue, {
  ...
  if (Number.isFinite(pct)) {
    ...
    this.sdPercentText.textContent = this._formatSigned(pct, { suffix: '%', fractionDigits: 2 });
  } else {
  ...
  async _loadFromSource(path) {
    try {
      ...
      if (typeof res === 'number' || typeof res === 'string') {
        ...
        if (res.changePercent != null) this.state.changePercent = String(res.changePercent);
        if (res.changeValue != null) this.state.changeValue = String(res.changeValue);
